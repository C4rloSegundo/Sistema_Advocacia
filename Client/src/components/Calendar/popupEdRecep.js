...
function TaskDialog({ open, onClose, onDelete, onSave, props }) {
  ...
  const [nomeAdvogado, setNomeAdvogado] = useState(props.event ? props.event.extendedProps.nomeAdvogado  : '');
  console.log(title)
  ...
  const handleNomeAdvogadoChange = (event) => {
    setNomeAdvogado(event.target.value);
  };
